...
achievements_data <- function(input, total_nasjonalt) {

  oppnaelse_teller <- total_nasjonalt %>%
    filter(HealthUnitShortName %in% c("Kristiansand", "Nasjonalt"),
           acc_overflyttet == 2,
           ed_tta_notrauma == 2,
           year == input$selected_year
    ) %>%
    group_by(HealthUnitShortName) %>%
    summarise(
      ...
      HealthUnitShortName = case_when(
        HealthUnitShortName == "Kristiansand" ~ "sykehus_1",
